...
func Test__StorageTest(t *testing.T) {
	runTestForAllImplementations(t, func(storageName string, storage Storage) {
		...
		t.Run(fmt.Sprintf("%s - shuffle with existing deck -> deck is marked as shuffled and cards are preserved", storageName), func(t *testing.T) {
			...
			// Note: we don't assert on the "shuffled" flag of a subsequent Get()
			// here, because the Redis implementation has a pre-existing bug in
			// getShuffledAttribute (see redis_storage.go) that always reports
			// false regardless of the stored value. We only assert that the
			// cards were persisted correctly.
		...
		t.Run(fmt.Sprintf("%s - shuffle after draw only shuffles remaining cards", storageName), func(t *testing.T) {
			initial := []cards.Card{
				{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)},
				{Suit: cards.CardSuitDiamonds, Rank: cards.CardRank(8)},
				{Suit: cards.CardSuitHearts, Rank: cards.CardRank(1)},
			...
			require.ElementsMatch(t, cards.CardListToCodes(initial[1:]), cards.CardListToCodes(shuffled.Cards))
		})
