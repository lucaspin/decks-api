...
func Test__StorageTest(t *testing.T) {
	runTestForAllImplementations(t, func(storageName string, storage Storage) {
		...
		t.Run(fmt.Sprintf("%s - shuffle with existing deck -> marks deck as shuffled and keeps the same cards", storageName), func(t *testing.T) {
			initial := []cards.Card{
				{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)},
				{Suit: cards.CardSuitDiamonds, Rank: cards.CardRank(8)},
		...
		t.Run(fmt.Sprintf("%s - shuffle after a draw keeps only the undrawn cards", storageName), func(t *testing.T) {
			initial := []cards.Card{
				{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)},
				{Suit: cards.CardSuitDiamonds, Rank: cards.CardRank(8)},
				{Suit: cards.CardSuitHearts, Rank: cards.CardRank(1)},
			...
			require.NoError(t, err)
			require.True(t, shuffled.Shuffled)
			require.Len(t, shuffled.Cards, len(remaining))
			require.ElementsMatch(t, remaining, shuffled.Cards)
			require.NotContains(t, shuffled.Cards, drawn[0])
		})
