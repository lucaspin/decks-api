...
func Test__StorageTest(t *testing.T) {
	runTestForAllImplementations(t, func(storageName string, storage Storage) {
		...
		t.Run(fmt.Sprintf("%s - shuffle with existing deck -> deck is marked as shuffled and keeps the same cards", storageName), func(t *testing.T) {
			generator := cards.NewCardGenerator()
			initial := generator.FullCardList()
			deck, err := storage.Create(context.Background(), initial, false)
			require.NoError(t, err)
		...
		t.Run(fmt.Sprintf("%s - shuffle after drawing -> only the remaining cards are shuffled", storageName), func(t *testing.T) {
			generator := cards.NewCardGenerator()
			initial := generator.FullCardList()
			deck, err := storage.Create(context.Background(), initial, false)
			require.NoError(t, err)

			drawn, err := storage.Draw(context.Background(), deck.DeckID, 10)
			require.NoError(t, err)
			require.Len(t, drawn, 10)

			shuffled, err := storage.Shuffle(context.Background(), deck.DeckID)
			require.NoError(t, err)
			require.True(t, shuffled.Shuffled)
			require.ElementsMatch(t, initial[10:], shuffled.Cards)
		})

		t.Run(fmt.Sprintf("%s - shuffle an empty deck -> no-op that still marks the deck as shuffled", storageName), func(t *testing.T) {
			initial := []cards.Card{{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)}}
			deck, err := storage.Create(context.Background(), initial, false)
			require.NoError(t, err)
