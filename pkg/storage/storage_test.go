...
func Test__StorageTest(t *testing.T) {
	runTestForAllImplementations(t, func(storageName string, storage Storage) {
		...
		t.Run(fmt.Sprintf("%s - shuffle with existing deck -> deck is marked as shuffled and card set is preserved", storageName), func(t *testing.T) {
			initial := []cards.Card{
				{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)},
				{Suit: cards.CardSuitDiamonds, Rank: cards.CardRank(8)},
			...
			// Note: we don't assert on the Shuffled flag returned by Get here,
			// because of a pre-existing bug in the redis implementation where
			// Get always reports shuffled=false (see redis_storage.go).
			// We do assert that the card set itself was correctly persisted.
			fetched, err := storage.Get(context.Background(), deck.DeckID)
			require.NoError(t, err)
			require.ElementsMatch(t, cards.CardListToCodes(initial), cards.CardListToCodes(fetched.Cards))
		})

		t.Run(fmt.Sprintf("%s - shuffle preserves remaining count after a draw", storageName), func(t *testing.T) {
			initial := []cards.Card{
				{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)},
				{Suit: cards.CardSuitDiamonds, Rank: cards.CardRank(8)},
				{Suit: cards.CardSuitHearts, Rank: cards.CardRank(1)},
			...
			require.ElementsMatch(t, cards.CardListToCodes(initial[2:]), cards.CardListToCodes(shuffled.Cards))
		})

		t.Run(fmt.Sprintf("%s - shuffling an empty deck -> no error, remaining 0, shuffled true", storageName), func(t *testing.T) {
			initial := []cards.Card{{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)}}
			deck, err := storage.Create(context.Background(), initial, false)
			require.NoError(t, err)
			...
			require.NoError(t, err)
			require.True(t, shuffled.Shuffled)
			require.Equal(t, 0, shuffled.Remaining())
		})
	})
}
