...
func Test__StorageTest(t *testing.T) {
	runTestForAllImplementations(t, func(storageName string, storage Storage) {
		...
		t.Run(fmt.Sprintf("%s - shuffle with existing deck -> shuffled flag is set and no cards are lost", storageName), func(t *testing.T) {
			initial := []cards.Card{
				{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)},
				{Suit: cards.CardSuitDiamonds, Rank: cards.CardRank(8)},
			...
			// the new card ordering is persisted.
			// Note: we don't assert on the `Shuffled` flag returned by Get here,
			// since RedisStorage.getShuffledAttribute has a pre-existing bug
			// that makes Get always report `Shuffled == false` - out of scope
			// for this change.
			fetched, err := storage.Get(context.Background(), deck.DeckID)
			require.NoError(t, err)
			require.ElementsMatch(t, initial, fetched.Cards)
		})

		t.Run(fmt.Sprintf("%s - shuffle after drawing cards -> only operates on remaining cards", storageName), func(t *testing.T) {
			initial := []cards.Card{
				{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)},
				{Suit: cards.CardSuitDiamonds, Rank: cards.CardRank(8)},
				{Suit: cards.CardSuitHearts, Rank: cards.CardRank(1)},
		...
		t.Run(fmt.Sprintf("%s - shuffle an empty deck -> success, remaining stays 0", storageName), func(t *testing.T) {
			initial := []cards.Card{{Suit: cards.CardSuitClubs, Rank: cards.CardRank(3)}}
			deck, err := storage.Create(context.Background(), initial, false)
			require.NoError(t, err)
