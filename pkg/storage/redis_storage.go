...
func (s *RedisStorage) Reshuffle(ctx context.Context, deckID *uuid.UUID, shuffle ShuffleFunc) (*Deck, error) {
	// We don't really need the shuffled attribute here,
	// but this is how we check that the deck exists before reshuffling it.
	_, err := s.getShuffledAttribute(ctx, deckID)
	if errors.Is(err, ErrDeckNotFound) {
		return nil, err
	}
	...
	// We know this is valid because we validate it before inserting.
	cardList, _ := cards.CodesToCardList(list)
	reshuffled := shuffle(cardList)

	// Rewrite the cards list with the new order.
	// If this fails, there's nothing meaningful to roll back,
	// since we haven't changed the shuffled key yet.
	if _, err := s.Client.Del(ctx, cardsKey).Result(); err != nil {
		return nil, err
	}

	if _, err := s.Client.RPush(ctx, cardsKey, cards.CardListToCodes(reshuffled)).Result(); err != nil {
		return nil, err
	}

	// Mark the deck as shuffled.
	// If this fails, we make a small effort
	// to restore the cards key to its previous order.
	shuffledKey := keyForAttribute(deckID, "shuffled")
	if _, err := s.Client.Set(ctx, shuffledKey, "true", 0).Result(); err != nil {
		if _, delErr := s.Client.Del(ctx, cardsKey).Result(); delErr != nil {
			log.Printf("Error rolling back key: %v", delErr)
		} else if _, pushErr := s.Client.RPush(ctx, cardsKey, cards.CardListToCodes(cardList)).Result(); pushErr != nil {
			log.Printf("Error rolling back key: %v", pushErr)
		}
