...
// Shuffle re-shuffles the cards currently remaining in the deck and marks it as shuffled.
//
// Note that the read-modify-write sequence below (LRange, then Del + RPush) is not atomic,
// consistent with the rest of this implementation - see the file header comment for details.
func (s *RedisStorage) Shuffle(ctx context.Context, deckID *uuid.UUID) (*Deck, error) {
	...
	// If the deck has no cards left, there's nothing to push back.
	if _, err := s.Client.Del(ctx, cardsKey).Result(); err != nil {
		return nil, err
	}
	...
	if _, err := s.Client.Set(ctx, shuffledKey, strconv.FormatBool(true), 0).Result(); err != nil {
		return nil, err
	}
