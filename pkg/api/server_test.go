...
func Test__ShuffleDeck(t *testing.T) {
	...
	t.Run("deck that exists -> 200 with proper response", func(t *testing.T) {
		...
		// The deck is persisted as shuffled, and still holds the full set of cards.
		response = execRequest(testServer, http.MethodGet, "/api/v1alpha/decks/"+deckID, nil)
		require.Equal(t, response.Code, 200)
		openResponse := &OpenDeckResponse{}
		require.NoError(t, json.NewDecoder(response.Body).Decode(&openResponse))
		...
		requireFullDeckAsSet(t, openResponse.Cards)
	})

	t.Run("deck with cards already drawn -> shuffles only the remaining cards", func(t *testing.T) {
		deckID := createDeck(t, testServer)

		response := execRequest(testServer, http.MethodPost, "/api/v1alpha/decks/"+deckID+"/draw?count=10", nil)
		require.Equal(t, response.Code, 200)
		...
		require.NoError(t, json.NewDecoder(response.Body).Decode(&shuffleResponse))
		require.True(t, shuffleResponse.Shuffled)
		require.Equal(t, 42, shuffleResponse.Remaining)

		response = execRequest(testServer, http.MethodGet, "/api/v1alpha/decks/"+deckID, nil)
		require.Equal(t, response.Code, 200)
		openResponse := &OpenDeckResponse{}
		require.NoError(t, json.NewDecoder(response.Body).Decode(&openResponse))
		require.Equal(t, 42, openResponse.Remaining)

		drawnCodes := make(map[string]bool, len(drawResponse.Cards))
		for _, c := range drawResponse.Cards {
			drawnCodes[c.Code] = true
		}

		for _, c := range openResponse.Cards {
			require.False(t, drawnCodes[c.Code], "card %s was drawn but is still present in the deck", c.Code)
		}
	})
}

func requireFullDeckAsSet(t *testing.T, list []Card) {
