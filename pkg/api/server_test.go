...
func Test__ShuffleDeck(t *testing.T) {
	...
	t.Run("deck that exists -> 200, deck is marked as shuffled, cards stay the same", func(t *testing.T) {
		deckID := createDeck(t, testServer)
		...
		// the deck now reports as shuffled and still has all of its cards
		response = execRequest(testServer, http.MethodGet, "/api/v1alpha/decks/"+deckID, nil)
		require.Equal(t, response.Code, 200)
		openResponse := &OpenDeckResponse{}
		require.NoError(t, json.NewDecoder(response.Body).Decode(&openResponse))
...
// requireSameCardsRegardlessOfOrder makes sure the given list contains
// exactly the 52 cards of a full deck, regardless of their order.
