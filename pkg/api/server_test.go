...
func Test__ReshuffleDeck(t *testing.T) {
	...
	t.Run("deck that exists -> 200, cards are reordered, and deck is marked as shuffled", func(t *testing.T) {
		deckID := createDeck(t, testServer)
		...
		requireSameCardsInAnyOrder(t, reshuffleResponse.Cards)

		// the deck itself reflects the new, shuffled state.
		response = execRequest(testServer, http.MethodGet, "/api/v1alpha/decks/"+deckID, nil)
		require.Equal(t, response.Code, 200)
		openResponse := &OpenDeckResponse{}
		require.NoError(t, json.NewDecoder(response.Body).Decode(&openResponse))
		require.True(t, openResponse.Shuffled)
		require.Equal(t, reshuffleResponse.Cards, openResponse.Cards)
	})

	t.Run("drawing after reshuffling only returns cards still in the deck", func(t *testing.T) {
		deckID := createDeck(t, testServer)
		...
		reshuffleResponse := &ReshuffleDeckResponse{}
		require.NoError(t, json.NewDecoder(response.Body).Decode(&reshuffleResponse))
		require.Equal(t, 0, reshuffleResponse.Remaining)
		require.Empty(t, reshuffleResponse.Cards)
	})
}
...
func requireSameCardsInAnyOrder(t *testing.T, list []Card) {
	codes := make([]string, len(list))
	for i, card := range list {
		codes[i] = card.Code
	}
