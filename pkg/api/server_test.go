...
func Test__ShuffleDeck(t *testing.T) {
	...
	t.Run("deck that exists -> 200, deck is shuffled and cards are preserved", func(t *testing.T) {
		deckID := createDeck(t, testServer)
		...
		// the deck is persisted as shuffled, with the same cards.
		response = execRequest(testServer, http.MethodGet, "/api/v1alpha/decks/"+deckID, nil)
		require.Equal(t, response.Code, 200)
		openResponse := &OpenDeckResponse{}
		require.NoError(t, json.NewDecoder(response.Body).Decode(&openResponse))
		...
		requireFullShuffledDeck(t, openResponse.Cards)
	})

	t.Run("shuffling a deck that already had cards drawn -> remaining cards are shuffled", func(t *testing.T) {
		deckID := createDeck(t, testServer)
