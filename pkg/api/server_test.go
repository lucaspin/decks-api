...
func Test__ShuffleDeck(t *testing.T) {
	...
	t.Run("deck that exists -> 200, cards reordered, remaining count unchanged", func(t *testing.T) {
		deckID := createDeck(t, testServer)
		...
		// opening the deck reflects the shuffle: same cards, different order, marked as shuffled.
		response = execRequest(testServer, http.MethodGet, "/api/v1alpha/decks/"+deckID, nil)
		require.Equal(t, response.Code, 200)
		openResponse := &OpenDeckResponse{}
		require.NoError(t, json.NewDecoder(response.Body).Decode(&openResponse))
		...
		requireSameCardsInDifferentOrder(t, openResponse.Cards)
	})

	t.Run("drawing after shuffling still works", func(t *testing.T) {
		deckID := createDeck(t, testServer)
...
func requireSameCardsInDifferentOrder(t *testing.T, list []Card) {
	codes := make([]string, len(list))
	for i, card := range list {
		codes[i] = card.Code
	}

	unshuffledOrder := []string{
		"AS", "2S", "3S", "4S", "5S", "6S", "7S", "8S", "9S", "10S", "JS", "QS", "KS",
		"AD", "2D", "3D", "4D", "5D", "6D", "7D", "8D", "9D", "10D", "JD", "QD", "KD",
		"AC", "2C", "3C", "4C", "5C", "6C", "7C", "8C", "9C", "10C", "JC", "QC", "KC",
		"AH", "2H", "3H", "4H", "5H", "6H", "7H", "8H", "9H", "10H", "JH", "QH", "KH",
	...
	require.ElementsMatch(t, unshuffledOrder, codes)
	require.NotEqual(t, unshuffledOrder, codes)
}
