...
func Test__ShuffleDeck(t *testing.T) {
	...
	t.Run("deck that exists -> 200, deck marked as shuffled with same cards in a new order", func(t *testing.T) {
		deckID := createDeck(t, testServer)
		...
		require.ElementsMatch(t, fullUnshuffledDeckCards(), shuffleResponse.Cards)
		require.NotEqual(t, fullUnshuffledDeckCards(), shuffleResponse.Cards)

		// the deck is now open with the shuffled order persisted
		openResponse := execRequest(testServer, http.MethodGet, "/api/v1alpha/decks/"+deckID, nil)
		require.Equal(t, openResponse.Code, 200)
		reopened := &OpenDeckResponse{}
		require.NoError(t, json.NewDecoder(openResponse.Body).Decode(&reopened))
		require.True(t, reopened.Shuffled)
		require.Equal(t, shuffleResponse.Cards, reopened.Cards)
	})
}
...
func fullUnshuffledDeckCards() []Card {
	codes := []string{
	...
	list := make([]Card, len(codes))
	for i, code := range codes {
		card, _ := cards.NewCardFromCode(code)
		list[i] = Card{
