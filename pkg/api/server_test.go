...
func Test__ShuffleDeck(t *testing.T) {
	...
	t.Run("deck that exists -> 200 with proper response, cards are shuffled", func(t *testing.T) {
		deckID := createDeck(t, testServer)
		...
		requireShuffledFullDeck(t, openResponse.Cards)
	})

	t.Run("draw then shuffle -> remaining reflects the draw, drawn cards stay out", func(t *testing.T) {
		deckID := createDeck(t, testServer)
		...
		drawnCodes := map[string]bool{}
		for _, c := range drawResponse.Cards {
			drawnCodes[c.Code] = true
		}
		...
		shuffleResponse := &ShuffleDeckResponse{}
		require.NoError(t, json.NewDecoder(response.Body).Decode(&shuffleResponse))
		require.Equal(t, 50, shuffleResponse.Remaining)

		response = execRequest(testServer, http.MethodGet, "/api/v1alpha/decks/"+deckID, nil)
		require.Equal(t, response.Code, 200)
		openResponse := &OpenDeckResponse{}
		require.NoError(t, json.NewDecoder(response.Body).Decode(&openResponse))
		require.Len(t, openResponse.Cards, 50)
		for _, c := range openResponse.Cards {
			require.False(t, drawnCodes[c.Code])
		}
	})

	t.Run("shuffle twice in a row -> still 200, still 52 cards", func(t *testing.T) {
		deckID := createDeck(t, testServer)
...
// Asserts that list contains the same 52 codes as a full deck, but not in the
// unshuffled order. With 52 cards, the probability of a false failure here is
// negligible.
func requireShuffledFullDeck(t *testing.T, list []Card) {
	codes := make([]string, len(list))
	for i, card := range list {
		codes[i] = card.Code
	}
	...
	require.ElementsMatch(t, unshuffled, codes)
	require.NotEqual(t, unshuffled, codes)
}
