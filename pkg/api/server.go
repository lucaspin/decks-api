...
func (s *Server) InitRouter() {
	...
	s.router.HandleFunc(basePath+"/decks/{deck_id}/shuffle", s.ShuffleDeck).Methods(http.MethodPost)
...
// ShuffleDeck re-shuffles the cards currently remaining in a deck, in place.
// The set of cards in the deck is not changed, only their order,
// and the deck is marked as shuffled.
func (s *Server) ShuffleDeck(w http.ResponseWriter, r *http.Request) {
	deckID, err := uuid.Parse(mux.Vars(r)["deck_id"])
	if err != nil {
		http.Error(w, "invalid deck ID", http.StatusBadRequest)
		return
	...
	if err != nil {
		...
		log.Printf("Unknown error fetching deck to shuffle: %v", err)
		http.Error(w, "unknown error", http.StatusInternalServerError)
		return
	}

	shuffled := s.generator.Shuffle(deck.Cards)

	deck, err = s.storage.Shuffle(r.Context(), &deckID, shuffled)
	if err == nil {
		response := newShuffleDeckResponse(deck)
		respondWithJSON(w, http.StatusOK, &response)
		return
	}
	...
	log.Printf("Unknown error shuffling deck: %v", err)
	http.Error(w, "unknown error", http.StatusInternalServerError)
}
...
func (s *Server) DeleteDeck(w http.ResponseWriter, r *http.Request) {
	...
	err = s.storage.Delete(r.Context(), &deckID)
	if err == nil {
	...
	log.Printf("Unknown error deleting deck: %v", err)
	http.Error(w, "unknown error", http.StatusInternalServerError)
}
