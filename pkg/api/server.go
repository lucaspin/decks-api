...
// Re-shuffles the cards currently in a deck, in place.
// This does not change which cards are in the deck, or how many of them are left -
// it only changes their order, and marks the deck as shuffled.
func (s *Server) ReshuffleDeck(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Printf("Unknown error fetching deck to reshuffle: %v", err)
		http.Error(w, "unknown error", http.StatusInternalServerError)
		return
	}

	newOrder := s.generator.Shuffle(deck.Cards)
	updated, err := s.storage.Shuffle(r.Context(), &deckID, newOrder)
	if err == nil {
		response := newReshuffleDeckResponse(updated)
		respondWithJSON(w, http.StatusOK, &response)
		return
	}
