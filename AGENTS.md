...
Persistence is done through the `storage.Storage` interface (`pkg/storage/storage.go`), with methods: `Create`, `Get`, `Draw`, `Shuffle`, `Delete`. The implementation is selected at startup by `storage.NewStorage()` based on the `DECK_STORAGE_TYPE` env var.
...
- Keep `docs/API.md` in sync with any change to API behavior, request/response shapes, or storage backends.
