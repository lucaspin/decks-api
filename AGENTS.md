...
Persistence is done through the `storage.Storage` interface (`pkg/storage/storage.go`), with methods `Create`, `Get`, `Draw`, `Delete`, and `Shuffle`. The implementation is selected at startup by `storage.NewStorage()` based on the `DECK_STORAGE_TYPE` env var.
